...
- **內勤後台的派遣單清單（`OrdersPanel`）只顯示「最新出貨日」那一批**（`onlyLatestDay()`，
  2026-08-07 起）：下班前匯入明天的資料之後，今天那批就不該再留在版面上（使用者原話：
  「只要有新日期匯入…就把舊日期的刪除，不要留在版面上」）。
  用「最新的一天」而不是「今天」，是因為明天的資料一匯進來就要能立刻勾選指派；
  在還沒匯明天的資料之前，今天的仍然看得到。**每個管道各自算自己的最新日期**
  （永昌可能停在 08/05，新竹已經是 08/07），不是取全域最大值。
  **「已刪除」分頁不套這個過濾**：那是回收桶，要查得到歷史。
- 客戶名稱旁的「派遣 MM/DD」小標顯示的是**出貨日期**（`DispatchDateTag` 的 `deliveryDate`），
  沒有 deliveryDate 的舊資料才退回用 `createdAt`。
