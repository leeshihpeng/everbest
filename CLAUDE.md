...
- `CarrierDispatch`：不再過濾 `COMPLETED`（貨運行的單子本來就每天清，不會累積）。
...
列入統計的條件寫在 `isActive()`。**已完成的單子也要算，保留最近 `STATS_KEEP_DAYS`（14）天**
（使用者 2026-08-05 要求，做法比照貨物追蹤）：

- 原本一送完就從統計消失，當天總量愈看愈少，跟實際出貨對不起來。
- 自家配送仍排除 `PENDING`（還沒指派出去的，首頁另有警告區塊在講）。
- **只是不列入統計，資料本身沒有刪除**——貨物追蹤是真的 purge，這裡刻意不那樣做，
  派遣單是營運紀錄，刪掉救不回來。
- 視窗以 `deliveryDate` 判斷，用 `withinStatsWindow()`（`components/common.tsx`），
  未來日期也算（預定明天送的要看得到）。
