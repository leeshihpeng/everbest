...
### 判斷「跟上次一樣」一定要比訂貨內容，不是只比客戶（2026-08-05 起）

同一張派遣單靠「配送方式＋送貨日期＋客戶代號」認定，但**認出是同一張之後，
還要比對品項與數量才知道內容有沒有變**（`itemsSignature()`，同品名合併數量後排序，與 CSV 列出順序無關）。

原本只要單子被動過（有勾檢貨／已檢貨／已完成）就整筆略過，於是**客戶臨時加訂或改量時，
那筆異動會被當成重複匯入而消失**，送貨人員照舊數量出貨——這正是使用者回報的誤差來源。現在：

- 內容完全相同 → `unchangedCount`，不動它（自動匯入每 15 分鐘重送的正常情況）。
- 內容有變且單子還在作業中 → 照常更新，但**保留已檢貨的勾選**：
  品名與數量都沒變的品項維持原勾選，新增或改量的品項一律回到未檢貨，
  送貨人員才看得出哪幾項需要重新備貨。已檢貨的單子多出未檢品項時退回 `SELECTED`。
- 內容有變但已 `COMPLETED`／`CANCELLED` → 不覆蓋（貨已送出／有人刻意刪掉），
  但**改為回報 `conflicts` 而不是靜默略過**，讓內勤知道要人工處理。
- 送貨人員已經在檢貨的單子被改 → 另外發通知給他，`changedInProgressCount` 也會回報。
  更新後內容就一致了，下一輪匯入判定為未變，所以**不會每 15 分鐘重覆通知**。

**不要為了省事把內容比對拿掉退回「只比客戶＋日期」**，那會讓加訂與改量再次無聲消失。
