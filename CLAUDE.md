...
- `WAREHOUSE`（倉管）：
  ...
  - 物流模式（`ManagerSelect`，路線排程系統底下）：**唯讀**。用 `canEdit = roles.includes("MANAGER")`
    收掉勾選框、指派送貨人員、優先標記、送出鈕；後端本來就擋（`/orders/select`、`PUT /orders/:id` 要 MANAGER，
...
- 找不到對應的人就留在 `PENDING` 並通知主管，**絕不硬塞給某個人**。
  物流管理的「派遣單勾選」變成純例外處理，平常是空的（畫面上有寫明原因，不要當成壞掉）。
- 匯入後會對每位被動到的送貨人員跑 `resequenceByCity()`：依 台北→新北→基隆→桃園→其他 重排
  `routeSequence` 並把 `routeOrderManual` 設為 true（＝照這個順序走，不自動重排最短路徑）。
  送貨人員畫面有「依縣市排序」／「改用最短路徑」兩顆按鈕可切換。
...
- **絕對不要改成真的刪除**：自動匯入每隔幾分鐘重讀同一份 ERP 檔案，
...
- `DispatchOrder.inRoute`＝送貨人員勾選「這趟送不送」。取消勾選的單子**留在名單上但不排進路線**，
  跟刪除是兩回事，不要混用。主管重新指派（`/orders/select`）會把它還原成 true。
